...
// nodeType 节点类型，设置了 4 种类型: 静态、根、命名参数捕获、任意参数捕获
type nodeType uint8
...
func (n *node) incrementChildPrio(pos int) int {
	...
	// adjust position (move to front)，按照优先级排序
	newPos := pos
...
func (n *node) addRoute(path string, handle Handle) {
	...
	walk:
		for {
			...
			// Find the longest common prefix. 寻找 path 与当前 node 的最长公共前缀
			...
			// Split edge
			// 如果最长公共前缀长度小于当前 node.path，说明是个新节点
			if i < len(n.path) {
				// 将当前节点作为 child 节点
				child := node{
					path:      n.path[i:],
			...
			// Make new node a child of this node. 给当前节点做一个新的子节点
			if i < len(path) {
				// 将 path 减去公共前缀
				path = path[i:]

				// 如果当前 node 的子节点为参数节点, 检查是否发生冲突
				if n.wildChild {
					// 将当前节点更新为他的第一个子节点
					n = n.children[0]
					n.priority++
				...
				// slash after param
				// 如果当前节点为参数节点，且 path 的第一个字符为 '/'，且只有一个子节点
